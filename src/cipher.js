window.cipher = {
  encode: (offset, string) => {
    ...
    for (let i = 0; i < string.length; i++) { //recorriendo el string ingresado para obtener el índice
      ...
      if (32 <= getAscii && getAscii <= 255) { // condicion para saber si es mayúscula 
        arrConvr += String.fromCharCode((getAscii - 32 + offset) % 222 + 32); // mostrar,concatenar, y fórmula cesar para cifrar la letra obtenida por en codigo Ascii
      }
  ...
  decode: (offset, string) => {
    ...
    for (let i = 0; i < string.length; i++) {  //recorriendo el string ingresado para obtener el índice
      ...
      if (32 <= getAsciii && getAsciii <= 255) { // condicion para saber si es mayúscula 
        arrDecode += String.fromCharCode(((getAsciii - 32 - offset) % 222) + 32); // mostrar,concatenar, y fórmula cesar para cifrar la letra obtenida por en codigo Ascii
      }
      // else
